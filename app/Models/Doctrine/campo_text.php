...
class CampoText extends Campo
{
    ...
    protected function display($modo, $dato, $etapa_id = false)
    {
        ...
        $display .= '<input id="' . $this->id . '" ' . ($modo == 'visualizacion' ? 'readonly' : '') . ' type="text" class="form-control has-error" name="' . $this->nombre . '" value="' . ($dato ? htmlspecialchars($dato->valor) : htmlspecialchars($valor_default)) . '" data-modo="' . $modo .'"placeholder="'.  $this->ayuda . '" />';
        $display .= '</div>';
