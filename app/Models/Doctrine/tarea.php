...
class Tarea extends Doctrine_Record
{
    ...
    //Obtiene el listado de usuarios que tienen acceso a esta tarea y que esten disponibles (no en vacaciones).
    // Muy similar a la query de arriba (mismos results)
    public function getUsuariosFromGruposDeUsuarioDeAsignador($etapa_id)
    {
        $query = Doctrine_Query::create()
            ->from('Usuario u, u.GruposUsuarios g, g.GrupoUsuariosHasUsuario gu')
            ->where('u.id = gu.usuario_id')
            ->andWhere('g.id = gu.grupo_usuarios_id')
            ;
        ...
        if ($this->acceso_modo == 'publico' || $this->acceso_modo == 'anonimo') {
            ...
            //Convertimos las variables e ids, separados por coma, en un arreglo de grupos de usuarios.
            $grupos_arr = array(-1);
            $grupos = explode(',', $this->grupos_usuarios);
            foreach ($grupos as $key => $g) {
                $r = new Regla($g);
            ...
            // grupos arr tiene los grupos que son asignables esta tarea
            $query->andWhereIn('g.id', $grupos_arr);
        }
