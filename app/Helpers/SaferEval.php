...
class SaferEval {
    ...
    function checkScript($code, $execute) {
        ...
        if (empty($this->errors) && !is_null($this->disallowedExpressions)){
        ...
        if (empty($this->errors)) {
            ...
            foreach ($this->tokens as $key => $token) {
                ...
                if (is_array($token)) {
                    ...
                    switch ($id) {
                        case('T_VARIABLE'):
                            if (!is_null($this->allowedVariables) && in_array($token[1], $this->allowedVariables) === false) {
                                $this->errors[$i]['name'] = 'Illegal variable: ' . $token[1];
                                $this->errors[$i]['line'] = $token[2];
                            }
                            break;
                        case('T_STRING'):
                            if (!is_null($this->allowedCalls) && in_array($token[1], $this->allowedCalls) === false) {
                                $this->errors[$i]['name'] = 'Illegal function: ' . $token[1];
                                $this->errors[$i]['line'] = $token[2];
                            }
                            break;
                        default:
                            if (!is_null($this->allowedTokens) && in_array($id, $this->allowedTokens) === false) {
