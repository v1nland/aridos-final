$(document).ready(function(){
    ...
    $("#draw").on("click",function(event){
        if(modo=="createBox"){
            ...
            $.post("/backend/procesos/ajax_crear_tarea/" + procesoId + "/" + id, "nombre=Tarea&posx=" + left + "&posy=" + top);
        }
    });
    $("#draw").on("click",".box",function(event){
        ...
        if(modo=="createConnection"){
            ...
            if(elements.length==2){
                ...
                //setJSPlumbEvents();
                $.post("/backend/procesos/ajax_crear_conexion/"+procesoId,"tarea_id_origen="+c.source+"&tarea_id_destino="+c.target+"&tipo="+c.tipo);
    ...
    $(document).on("dblclick doubletap","#draw .box",function(event){
        var id=$(this).attr("id");
        $('#modal').load("/backend/procesos/ajax_editar_tarea/"+procesoId+"/"+id);
        $('#modal').modal('show')
    });
    ...
    $(document).on("dblclick doubletap","#draw ._jsPlumb_connector",function(event){
        ...
        $(connections).each(function(i,connection){
            if(connection.canvas==conectorSeleccionado){
                ...
                $('#modal').load("/backend/procesos/ajax_editar_conexiones/"+procesoId+"/"+id);
                $('#modal').modal('show')
            }
        });
    });
    ...
    $(document).on("click","#areaDibujo h1 a",function(event){
        $('#modal').load("/backend/procesos/ajax_editar/"+procesoId);
        $('#modal').modal('show');
        return false;
    });
...
function dblClickConnectionEvent(connection){
    ...
    $('#modal').load("/backend/procesos/ajax_editar_conexiones/"+procesoId+"/"+tareaOrigenId);
    $('#modal').modal('show')
}
